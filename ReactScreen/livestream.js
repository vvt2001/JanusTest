...
function startFacecam(track) {
  try {
    ...
    if (placeholder) {
      ...
      placeholder.userId = currentUserId;
...
function switchRemoteFeed(placeholderIndex, oldId, newId) {
  let update = {
    ...
    subscribe: [
      {
        feed: parseInt(newId, 10),
      },
    ],
    unsubscribe: [
      {
        feed: parseInt(oldId, 10),
      },
    ],
  };
...
function canvasJoining() {
  ...
  console.log('canvas join request:', join);
    ...
    onmessage: function (msg, jsep) {
      ...
      if (jsep) {
        remotePlugin.createAnswer({
          ...
          tracks: [{type: 'data'}],
          success: function (jsep) {
            let body = {request: 'start', room: parseInt(currentRoomId, 10)};
...
function joinAudioRoom() {
  if (currentUserId && currentRoomId && currentUserName) {
    ...
    console.log('audiobridge joinRequest', register);

    audiobridgePlugin.send({message: register});
  } else {
    console.log('Missing current userid or roomid for audio room'); // Alert if the field is empty
  }
}
...
function joinVideoRoom() {
  if (currentUserId && currentRoomId && currentUserName) {
    ...
    console.log('videoroom join request:', join);

    videoRoomPlugin.send({message: join});
  } else {
    console.log('Missing current userid or roomid for video room'); // Alert if the field is empty
  }
}
